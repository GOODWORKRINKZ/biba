...
|--------|--------|--------------|
| M1 | D1 вертикально у клемм, R27 вправо, C15 обратно влево | y 55…76 |
| M2 | D2, R28, C16 — то же | y 88…109 |
| Питание | PM2 (альтернативный DC/DC) | y 117…138 |

Шины M+ и M− идут параллельно и нигде не пересекаются: от клеммы вправо к
супрессору, дальше через резистор к плёнке и обратно понизу.
...
- **VIN у C10, C11, C12** — прямая трасса к выводу 7 драйвера режет веер
  сигнальных выводов 2–6. Нужен обход низом (переходное → B.Cu под веером →
  переходное в полигон VIN) или ручная трасса в pcbnew. У C13 путь был
  свободен, он разведён.
- **+5V и SW у PM2** — идут через всю плату к левой части, это полноценная
  ручная трассировка.
