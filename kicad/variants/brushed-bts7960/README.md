...
| `brushed-bts7960.kicad_sch` | схема 1:1 с оригиналом: 57 компонентов, 56 сетей |
| `brushed-bts7960.kicad_pcb` | плата 124.51 × 100.33 мм, 2 слоя, 539 дорожек, 51 via, 16 полигонов |
...
Генераторы (перезапуск идемпотентен):
...
- **Схема и плата сходятся**: одинаковый набор из 57 обозначений,
  футпринт в схеме совпадает с футпринтом на плате у всех компонентов,
  все ссылки `biba:*` разрешаются в `biba.pretty`.
...
1. Расставить `PWR_FLAG` в схеме — закрыть 2 ошибки ERC.
2. Разобрать шелкографию (234 предупреждения) — перенос подписей.
3. Решить по 0805/0603: оставить их без паяльной пасты или вернуть апертуры.
4. Подтянуть 3D-модели в `kicad/common/3dmodels/`.
5. Экспорт герберов/BOM для производства из KiCad вместо архива EasyEDA.
