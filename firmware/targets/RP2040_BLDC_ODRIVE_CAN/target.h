 ...
 * controllers + CAN link.  The same drivers over the ASCII UART link
 * are a separate target, RP2040_BLDC_ODRIVE_UART, because the wiring
 * genuinely differs (MCP2515 on SPI0 vs. two UART1 pins).
 *
 ...
 * the brushed BTS7960 boards of the existing RP2040_DC_BTS7960_PWM target.
 *
...
#define BIBA_TARGET_NAME            "RP2040_BLDC_ODRIVE_CAN"

/* --- Capability flags ------------------------------------------------- */

/* BTS7960 driver removed: the drive API is backed by
 * drivers/odrive_can.c behind the drivers/bldc.h contract. */
 ...
 * and src/drivers/bts7960.c is excluded by [rp2040_bldc_odrive_can_src_filter].
 *
...
/* --- ODrive UART-A --------------------------------------------------- *
 *
 * Not wired on this target: the ASCII link is its own target,
 * RP2040_BLDC_ODRIVE_UART (GP4/GP5, drivers/odrive_uart.c).  GP4 and
 * GP5 stay free here so a bench operator can solder the UART on
 * temporarily without disturbing the CAN wiring.
 */
