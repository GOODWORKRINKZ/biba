...
/* Target-specific overrides for RP2040_BLDC_ODRIVE_CAN.
 *
 ...
 *
 * Fixed to CAN on this target — the link is part of the target name
 * now (ADR-0002 §1).  drivers/odrive_can.c provides drivers/bldc.h;
 * drivers/odrive_uart.c compiles to nothing, so there is no linker
 * conflict even though both are in the src_filter.
 *
 * Want the ASCII link instead?  Build RP2040_BLDC_ODRIVE_UART — it is
 * the preferred production transport, because the CANSimple stack in
 * ODrive fw 0.5.6 silently wedges at idle.
 */
