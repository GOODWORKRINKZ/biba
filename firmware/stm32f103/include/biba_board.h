...
/* STM32F103C8T6 ("Blue Pill") pin map for BiBa.
 *
 * Pinout rationale and peripheral assignment is documented in
 * docs/stm32_architecture.md and the "Подключение STM32F103" section of
 * docs/wiring.md. The short summary:
 *
 *   - TIM1 owns all four BTS7960 PWM channels so the four lines stay in
 *     phase and can get dead-time between RPWM/LPWM of the same side.
 *   - USART1 is deliberately unused: its AF pins PA9/PA10 are taken by PWM.
 *   - ADC1 runs a circular DMA scan over PA0..PA6.
 *   - SPI2 is the slave link to the SBC (PB12..PB15).
 *   - USART3 (PB10/PB11) is the CRSF UART at 420000 baud via DMA.
 *   - I2C1 (PB6/PB7) is the IMU bus.
 *   - JTAG is released at boot (SW-DP only) so PB3/PB4/PA15 are free GPIOs.
 */

/* STM32 HAL-style macros. We guard inclusion so this header stays usable
 * from native_test (where STM32 HAL headers are not present). */
