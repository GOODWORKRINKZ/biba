...
def make_signal(
    ...
    of the 2026-05-22 hardware capture.  Frequency range 180–940 Hz matches
    the calibrated plant: K=10.13 Hz/%, dead_zone=74.6 Hz, so duty
    25/50/75/100 % → ~178/432/686/940 Hz.
    """
...
# RPM-band frequencies matching calibrated plant K=10.13 Hz/%:
# duty 25/50/75/100 % → approx 178/432/686/940 Hz.
@pytest.mark.parametrize("freq", [180, 430, 680, 940])
...
def test_fft_ignores_dc_offset() -> None:
    # 430 Hz fundamental (duty ~50 %) with extra +1000 LSB DC offset.
    sig = make_signal(430.0) + 1000.0
    est = freq_fft(sig, 10000)
    assert abs(est - 430.0) / 430.0 < 0.05, f"FFT leaked to DC: est={est}"
