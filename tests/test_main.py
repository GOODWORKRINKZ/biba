...
def test_throttle_direction_guard_blocks_reverse_until_neutral() -> None:
    main = importlib.import_module("main")
    guard = main._ThrottleDirectionGuard(deadband=0.05)

    assert guard.update(-1.0) == pytest.approx(-1.0)
    assert guard.update(1.0) == 0.0
    assert guard.update(0.01) == 0.0
    assert guard.update(1.0) == pytest.approx(1.0)
...
def test_main_blocks_reverse_without_neutral_sample(monkeypatch: pytest.MonkeyPatch) -> None:
    main = importlib.import_module("main")
    drive_calls: list[tuple[float, float, float]] = []
    ...
    monkeypatch.setattr(main.config, "THROTTLE_REQUIRES_NEUTRAL_BEFORE_REVERSE", True)
    monkeypatch.setattr(main, "RUNNING", True)

    assert main.main() == 0
    assert drive_calls[0][0] == pytest.approx(-1.0)
    assert drive_calls[1][0] == 0.0
