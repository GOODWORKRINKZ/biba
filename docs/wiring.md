...
Текущая распиновка выше поддерживает одновременную работу двух BTS7960 на Pi Zero 2W в режиме hardware PWM, но не в старой модели "независимый `RPWM/LPWM` duty на каждом моторе".

- Левый мотор использует `RPWM=12` и `LPWM=18`.
- Правый мотор использует `RPWM=19` и `LPWM=13`.
- Для левого мотора обе PWM-линии сидят на hardware PWM channel 0.
- Для правого мотора обе PWM-линии сидят на hardware PWM channel 1.
- Поэтому hardware-режим BTS7960 должен использовать один общий hardware PWM duty на мотор и выбирать направление через `REN/LEN`.
...
Кодовый дефолт `BTS7960_PWM_MODE=HARDWARE` соответствует текущей проводке и новой семантике драйвера.
...
В текущей разводке один мотор использует оба вывода одного аппаратного канала. Поэтому для режима `HARDWARE` драйвер делает так:

- Forward: одинаковый PWM на `RPWM` и `LPWM`, `REN=1`, `LEN=0`
- Reverse: одинаковый PWM на `RPWM` и `LPWM`, `REN=0`, `LEN=1`
- Stop: `RPWM=0`, `LPWM=0`, `REN=0`, `LEN=0`

Плавность остановки и реверса задаётся не здесь, а рампой в `DifferentialDrive`, которая уже доводит команду до нуля перед сменой направления.
