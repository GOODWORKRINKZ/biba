...
> Этот документ описывает только STM32-сторону. Общую картину трёх композиций
> робота и роль STM32 в каждой смотрите в [system_architecture.md](system_architecture.md).
> Соответствие режимов прошивки и композиций:
>
> - **Standalone** — композиция B (STM32-only, без SBC).
> - **Companion** — композиция C (Pi + STM32, ROS2-стек на SBC).
> - **Combined** — отладочный гибрид; в продакшене не используется.
...
> Соответствие композициям робота: **Standalone = композиция B**, **Companion = композиция C**. См. [system_architecture.md](system_architecture.md#три-композиции-робота).

### Standalone (композиция B)
...
Единая прошивка, режим выбирается при старте по пину `MODE_SEL`
(`pull-up`; замыкание на GND = companion). Сам пин зависит от
выбранного target'а: на `BLUEPILL_F103C8` это PB9, на `BIBA_F103_REV_A`
и его клоне живёт PA7 (PB9 там занят под TIM4_CH4). Это позволяет
одной прошивкой закрывать обе сборки робота.
...
| Target                  | Плата                                              |
| ----------------------- | --------------------------------------------------- |
| `BLUEPILL_F103C8`       | Стоковая "Blue Pill" с настоящим чипом (20 КБ ОЗУ) |
| `BLUEPILL_F103C8_CLONE` | Та же распиновка, но клон-чип с 8 КБ ОЗУ         |
| `BIBA_F103_REV_A`       | Пример кастомного PCB (прототип Rev A)         |

Вариант `_CLONE` не имеет своей директории в `targets/` — он
переиспользует `BLUEPILL_F103C8/target.h` буква в букву и
отличается лишь линкер-скриптом (`firmware/ldscripts/stm32f103_8k_ram.ld`,
`LENGTH(RAM) = 8K`) и лимитом RAM в PlatformIO. Стоковый CMSIS-линкер
кладёт `_estack` на `0x20005000`, что на клоне выходит за пределы
реального ОЗУ и роняет MCU в HardFault на первом же прерывании.
...
│               crsf, imu                                  │
├──────────────────────────────────────────────────────────┤
│ hal/          biba_hal: clock, GPIO, motor PWM, ADC+DMA, │
...
Всё, что трогает регистры STM32, лежит в `hal/biba_hal.c` (плюс
`hal/biba_hal_motor.c` для мотор-PWM и motor-audio, и
`hal/biba_hal_debug.c` для семихостинг-логирования при отладке) и
исключается из native-сборки через `build_src_filter` в
`platformio.ini`.
...
- **Motor PWM** — распиновка зависит от target'а:
  - `BLUEPILL_F103C8`: все 4 канала на **TIM1** (PA8..PA11), общий
    несущий carrier ⇒ один набор частот для всех 4 линий.
  - `BIBA_F103_REV_A`: каждая из 4 линий висит на **своём таймере**
    (TIM1/TIM2/TIM3/TIM4). В тяговом режиме HAL держит одинаковый ARR
    на всех, в audio-режиме ARR/PSC/CCR программируются независимо —
    это и есть motor-audio через BTS7960 (у BiBa нет buzzer'а).
    Флаг target'а: `BIBA_TARGET_HAS_PER_CHANNEL_TIMER_PWM`.
...
| 0x40 | `SET_MOTOR_AUDIO`| `uint16 freq_hz[4]; uint8 duty_q8[4]; uint8 flags` |

`SET_MOTOR_AUDIO` адресует 4 канала PWM (L_RPWM, L_LPWM, R_RPWM,
R_LPWM) и применим только к target'ам с
`BIBA_TARGET_HAS_PER_CHANNEL_TIMER_PWM == 1` — на остальных firmware
молча игнорирует команду. Звука через buzzer у BiBa нет: тон даётся
напрямую через ключи BTS7960.
...
│   ├── drivers/             # bts7960, current/voltage_sense, crsf, imu
│   ├── hal/biba_hal.*       # STM32Cube wrapper (+ biba_hal_motor.c, biba_hal_debug.c)
...
└── test/
    ...
    └── test_support/        # общий shim для native / Unity (+ unity_config.h)
```
