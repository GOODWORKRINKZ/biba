...
Важный нюанс: hardware-режим для BTS7960 теперь не пытается независимо управлять `RPWM` и `LPWM` внутри одного мотора. В этой проводке обе PWM-линии каждого мотора сидят на одном аппаратном канале Raspberry Pi, поэтому low-level driver подаёт одинаковый hardware PWM duty на обе линии, а направление выбирает через `REN` и `LEN`.

Плавная остановка и мягкий реверс по-прежнему обеспечиваются рампой уровнем выше, в `DifferentialDrive`; low-level driver только отражает знак и величину уже рассчитанной команды.
