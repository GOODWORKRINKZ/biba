...
> **Статус:** исследование под новый firmware-таргет `RP2040_BLDC_ODRIVE_CAN` (ветка для работы с BLDC-моторами через MCP2515 → CAN).
...
Текущая ветка `rp2040-port` управляет щёточными DC-моторами через BTS7960 (см. `firmware/targets/RP2040_DC_BTS7960_PWM/target.md` и `docs/wiring.md`). У BTS7960 зафиксирован тепловой режим — дешёвые модули без радиатора перегреваются после ~20-30 мин интенсивной езды (см. `.planning/PROJECT.md` → Context, и Phase 4 UAT).
...
GP22 уже занят под `IMU_INT1` в существующем таргете `RP2040_DC_BTS7960_PWM` (см. `firmware/targets/RP2040_DC_BTS7960_PWM/target.md` §5). Новый target `RP2040_BLDC_ODRIVE_CAN` стартует с чистого `target.h`, поэтому может свободно выбрать любой GPIO. Рекомендуемая раскладка под RP2040 SPI0:
...
- В `RP2040_BLDC_ODRIVE_CAN` target header'е имеет смысл оставить UART1 (GP4/GP5) как `ODRIVE_UART_TX/RX` и заложить опциональный код-путь для ASCII-протокола, активируемый через Kconfig (`-DODRIVE_LINK=UART`).
...
## 9. Рекомендуемая распиновка таргета `RP2040_BLDC_ODRIVE_CAN`
...
- `firmware/targets/RP2040_DC_BTS7960_PWM/target.md` — текущая распиновка RP2040-таргета
...
Дальнейшие шаги — в задаче `t_3cca56ce` (Architecture нового таргета) и `t_a3ed6b73` / `t_6c05e712` / `t_add4db37` (зависит от декомпозиции). Настоящий документ служит опорой для ADR и `target.md` нового таргета `RP2040_BLDC_ODRIVE_CAN`.